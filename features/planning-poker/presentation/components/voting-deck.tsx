...
export function VotingDeck({
  ...
  return (
    <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4 backdrop-blur-xl sm:p-5">
      ...
      <div className="mt-4 grid grid-cols-4 gap-2 sm:grid-cols-5 sm:gap-2.5 md:grid-cols-6 lg:grid-cols-5 xl:grid-cols-6">
        {room.deck.values.map((card) => {
          ...
          return (
            <button
              ...
              onClick={() => onVote(card)}
              className={`group relative aspect-[3/4] overflow-hidden rounded-xl text-lg font-bold transition-all enabled:hover:-translate-y-0.5 enabled:hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-40 sm:rounded-2xl sm:text-xl ${
                selected
